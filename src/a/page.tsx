...
export default function ModuloA() {
  ...
  // Shims puente al host: los callsites (api.get / router.push / guardarProgresoTps) quedan
  // idénticos, pero resuelven al adaptador inyectado. No se reescribe la lógica del wizard.
  ...
  useEffect(() => {
    ...
    if (!preguntas?.A?.length) { router.replace('/cuestionario'); return }
  ...
  function seleccionar(valor: number) {
    ...
    if (step < items.length - 1) {
      ...
      setListo(true)
      if (session) guardarProgresoTps(session.asesor, 25)
      setTimeout(() => router.push('/cuestionario/b'), 400)
    }
  }

  function salir() {
    if (session) guardarProgresoTps(session.asesor, 10)
    onExit()
  }

  if (!items.length) return <Pantalla />
    ...
    <Pantalla modulo="A" step={step + 1} total={total} progreso={progreso} onExit={salir}>
      <div style={{ marginBottom: 12 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 10, padding: '0 4px' }}>
          <span style={{ fontSize: 10, color: '#8a8885' }}>1 = muy parecido al texto de arriba</span>
          <span style={{ fontSize: 10, color: '#8a8885' }}>4 = muy parecido al texto de abajo</span>
        </div>
      </div>
    </Pantalla>
  )
...
function Pantalla({ modulo, step, total, progreso, onExit, children }: {
  modulo?: string; step?: number; total?: number; progreso?: number; onExit?: () => void; children?: React.ReactNode
}) {
  return (
      ...
      <div style={{ background: '#0b0a09', padding: '14px 20px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
          <div>
            ...
            {modulo && <div style={{ fontSize: 14, fontWeight: 700, color: '#fff', marginTop: 2 }}>
              Módulo {modulo} — Iniciativa
            </div>}
          </div>
        ...
        <div style={{ height: 3, background: 'rgba(255,255,255,0.1)', borderRadius: 2 }}>
          <div style={{ height: '100%', background: '#cbf135', borderRadius: 2, width: `${progreso ?? 0}%`, transition: 'width 0.3s ease' }} />
        </div>
      </div>
